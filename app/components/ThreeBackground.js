...
function buildScene(T, noise, dk, mobile) {
  ...
  scene.fog = new T.FogExp2(dk ? 0x060c1a : 0x8ec8f0, dk ? 0.014 : 0.0085);
  const refs = {};
  ...
  createRabbits(T, scene, dk, mobile, refs);
...
function setupSky(T, scene, dk, refs) {
  ...
  if (dk) {
    gr.addColorStop(0, '#010308'); gr.addColorStop(0.25, '#080e1e');
    gr.addColorStop(0.55, '#0b1226'); gr.addColorStop(1, '#0f1628');
  } else {
    gr.addColorStop(0, '#1a66cc'); gr.addColorStop(0.18, '#3a8de0');
    gr.addColorStop(0.45, '#6ab8f5'); gr.addColorStop(0.72, '#a4d8f4');
    gr.addColorStop(1, '#c5e8d8');
  }
...
function setupLighting(T, scene, dk, refs, mobile) {
  const hemi = new T.HemisphereLight(dk ? 0x1c2a50 : 0xb8d8ff, dk ? 0x050a0a : 0x527020, dk ? 0.55 : 1.0);
  scene.add(hemi); refs.hemiLight = hemi;

  const ambient = new T.AmbientLight(dk ? 0x0d1525 : 0xffe8c0, dk ? 0.4 : 0.55);
  scene.add(ambient); refs.ambientLight = ambient;

  const sun = new T.DirectionalLight(dk ? 0x6688bb : 0xffd080, dk ? 0.7 : 1.6);
  sun.position.set(dk ? 12 : -14, 18, 6);
  sun.castShadow = true;
  sun.shadow.mapSize.set(mobile ? 512 : 1024, mobile ? 512 : 1024); // reduced: was 1024/2048
  sun.shadow.camera.near = 1; sun.shadow.camera.far = 90;
  sun.shadow.camera.left = -32; sun.shadow.camera.right = 32;
  sun.shadow.camera.top = 28;  sun.shadow.camera.bottom = -12;
  sun.shadow.bias = -0.0008; sun.shadow.normalBias = 0.04;
  scene.add(sun); refs.sun = sun; refs.sunDayI = dk ? 0.7 : 1.6;

  if (dk) {
    const rim = new T.DirectionalLight(0x2233aa, 0.3); rim.position.set(-12, 6, -12); scene.add(rim);
    const glow = new T.PointLight(0x4466cc, 0.5, 60); glow.position.set(10, 12, -8); scene.add(glow);
  } else {
    const wf = new T.DirectionalLight(0xffc890, 0.35); wf.position.set(14, 8, 8); scene.add(wf);
  }
  refs.dayFogColor   = new T.Color(0x8ec8f0);
  refs.nightFogColor = new T.Color(0x060c18);
  refs._lastDN = -1; // for throttle
}
...
// ===========================================================================
// GROUND — reduced amplitude (max 0.35) + foreground z-fade so road sits cleanly
// ===========================================================================
function setupGround(T, scene, noise, dk, mobile, refs) {
  // Reduced segments: was 120×80, now 70×45 — still enough for flat-shading
  const segsW = mobile ? 48 : 70, segsD = mobile ? 32 : 45;
  const geo = new T.PlaneGeometry(160, 90, segsW, segsD);
  ...
  const darkC  = new T.Color(dk ? 0x0a1a0c : 0x54a024); // flat areas use medium-bright green
  const lightC = new T.Color(dk ? 0x142410 : 0x7ed038); // bumps use a vivid highlight green
  ...
  for (let i = 0; i < count; i++) {
    ...
    const h = Math.max(0,
      noise.fbm((x + sX) * 0.14, (z + sZ) * 0.14, 3) * 0.28 +
      noise.fbm((x + sX) * 0.32, (z + sZ) * 0.32, 2) * 0.12 +
      noise.fbm((x + sX) * 0.68, (z + sZ) * 0.68, 1) * 0.05
    );

    pos.setY(i, h);
    // Minimum blend of 0.35 so perfectly flat areas near the camera
    // still render as a medium green rather than the darkest shade
    tmp.lerpColors(darkC, lightC, Math.min(1, Math.max(0.35, h / 0.25)));
  ...
  const mesh = new T.Mesh(geo, new T.MeshStandardMaterial({ vertexColors: true, roughness: 0.92, flatShading: true }));
...
function createHills(T, scene, noise, dk, mobile, refs) {
  refs.hills = [];
  const layers = [
    { z:-28, w:140, d:30, sw:mobile?36:72, sd:mobile?10:20, bY:-1.5, amp:3.8, fr:0.18, oct:4, cL:new T.Color().setHSL(0.60,0.28,0.44), cD:new T.Color().setHSL(0.60,0.22,0.09) },
    { z:-16, w:120, d:28, sw:mobile?42:80, sd:mobile?12:24, bY:-1.8, amp:2.8, fr:0.22, oct:4, cL:new T.Color().setHSL(0.38,0.40,0.36), cD:new T.Color().setHSL(0.38,0.30,0.07) },
    { z: -6, w:100, d:24, sw:mobile?42:80, sd:mobile?12:22, bY:-2.0, amp:2.2, fr:0.26, oct:3, cL:new T.Color().setHSL(0.28,0.55,0.30), cD:new T.Color().setHSL(0.28,0.30,0.06) },
  ];
  layers.forEach((cfg, li) => {
    const geo = new T.PlaneGeometry(cfg.w, cfg.d, cfg.sw, cfg.sd);
    geo.rotateX(-Math.PI / 2);
    const pos = geo.attributes.position, ox = (Math.random()-0.5)*20, oz = (Math.random()-0.5)*5;
    for (let i = 0; i < pos.count; i++)
      pos.setY(i, Math.max(0, noise.fbm((pos.getX(i)+ox)*cfg.fr, (pos.getZ(i)+oz)*cfg.fr, cfg.oct)) * cfg.amp);
    geo.computeVertexNormals();
    const mesh = new T.Mesh(geo, new T.MeshStandardMaterial({ color: dk?cfg.cD:cfg.cL, roughness:0.88, flatShading:true }));
...
// ===========================================================================
// WATER — store vertex X/Z once so the per-frame loop can use direct array access
// ===========================================================================
function createWater(T, scene, dk, refs) {
  const geo = new T.PlaneGeometry(20, 14, 20, 20); // reduced segs: was 28×28
  geo.rotateX(-Math.PI / 2);

  // Cache X and Z values — they never change, only Y ripples
  const pos = geo.attributes.position;
  const n = pos.count;
  const wXZ = new Float32Array(n * 2); // interleaved x,z
  for (let i = 0; i < n; i++) { wXZ[i*2] = pos.getX(i); wXZ[i*2+1] = pos.getZ(i); }

  const mat = new T.MeshStandardMaterial({ color: dk?0x102840:0x48aaee, transparent:true, opacity:dk?0.70:0.55, roughness:0.15, metalness:0.2, side:T.DoubleSide });
  const mesh = new T.Mesh(geo, mat); mesh.position.set(18, -1.85, -2); scene.add(mesh);
  refs.water = mesh; refs.waterXZ = wXZ; refs.waterN = n;

  const rg = new T.RingGeometry(7, 7.3, 28); rg.rotateX(-Math.PI/2);
  const ring = new T.Mesh(rg, new T.MeshBasicMaterial({color:dk?0x1a3040:0x68bb44,transparent:true,opacity:0.4}));
  ring.position.set(18, -1.84, -2); scene.add(ring);
}
...
function createTrees(T, scene, dk, mobile, refs) {
  refs.trees = [];
  const trunkMat = new T.MeshStandardMaterial({ color:dk?0x1a2020:0x7a5020, roughness:0.9, flatShading:true });
  const tCount = mobile ? 16 : 40; // reduced: was 18/48
  for (let i = 0; i < tCount; i++) {
    const tx=(Math.random()-0.5)*65, tz=-3-Math.random()*30, tH=1.2+Math.random()*3.0;
    const trunk=new T.Mesh(new T.CylinderGeometry(0.07,0.15,tH,5),trunkMat);
    trunk.position.set(tx,-2.2+tH/2,tz); trunk.castShadow=true; scene.add(trunk);
    if (Math.random()>0.38) {
      for (let l=0;l<3;l++) {
        const cr=(0.95-l*0.22)*(0.8+Math.random()*0.4), ch=(1.15-l*0.18)*(0.8+Math.random()*0.4);
        const col=dk?new T.Color().setHSL(0.34,0.28,0.04+Math.random()*0.025):new T.Color().setHSL(0.30+Math.random()*0.07,0.62,0.22+Math.random()*0.12);
        const cone=new T.Mesh(new T.ConeGeometry(cr,ch,6+Math.floor(Math.random()*3)),new T.MeshStandardMaterial({color:col,roughness:0.85,flatShading:true}));
        cone.position.set(tx,-2.2+tH+l*0.60+0.25,tz); cone.castShadow=true;
        cone.userData={baseX:tx,sway:0.03+Math.random()*0.05,freq:0.28+Math.random()*0.45};
        scene.add(cone); refs.trees.push(cone);
      }
    } else {
      const cr=0.55+Math.random()*0.95;
      const col=dk?new T.Color().setHSL(0.32,0.22,0.04+Math.random()*0.03):new T.Color().setHSL(0.26+Math.random()*0.10,0.56,0.26+Math.random()*0.16);
      const crown=new T.Mesh(new T.SphereGeometry(cr,7,5),new T.MeshStandardMaterial({color:col,roughness:0.85,flatShading:true}));
      crown.position.set(tx,-2.2+tH+cr*0.42,tz); crown.castShadow=true;
      crown.userData={baseX:tx,sway:0.06+Math.random()*0.10,freq:0.35+Math.random()*0.55};
...
function createRabbits(T,scene,dk,mobile,refs){
  refs.rabbits=[];
  for(let i=0;i<(mobile?2:4);i++){
    const g=new T.Group(), bC=dk?0x888898:0xd8c8a8, hC=dk?0x9090a0:0xe0d0b0, eC=dk?0xa0a0b0:0xe8d8b8;
    g.add(new T.Mesh(new T.SphereGeometry(0.14,6,4),new T.MeshStandardMaterial({color:bC})));
    const hd=new T.Mesh(new T.SphereGeometry(0.09,5,3),new T.MeshStandardMaterial({color:hC})); hd.position.set(0.13,0.12,0); g.add(hd);
    const eM=new T.MeshStandardMaterial({color:eC}); [-0.04,0.04].forEach(ez=>{const e=new T.Mesh(new T.CylinderGeometry(0.015,0.022,0.13,4),eM);e.position.set(0.12,0.25,ez);g.add(e);});
    g.position.set((Math.random()-0.5)*34,-1.90,(Math.random()-0.5)*18);
    g.userData={bx:g.position.x,bz:g.position.z,sp:0.8+Math.random()*1.2,ph:Math.random()*6.28,hop:0.14+Math.random()*0.1};
    scene.add(g); refs.rabbits.push(g);
  }
}
...
function animateScene(T, { scene, refs }, t, dk, dayNight, renderer) {
  ...
  // ── Rabbits hop ──────────────────────────────────────────────────────
  for (const r of refs.rabbits) {
    const d=r.userData;
    r.position.y = -1.90 + Math.abs(Math.sin(t*d.sp+d.ph))*d.hop;
    r.position.x = d.bx + Math.sin(t*d.sp*0.3+d.ph)*2.4;
    r.position.z = d.bz + Math.cos(t*d.sp*0.2+d.ph)*1.8;
  }

  // ── Water ripple — direct array access, NO computeVertexNormals ──────
  if (refs.water) {
    const wa = refs.water.geometry.attributes.position.array;
    const xz = refs.waterXZ, n = refs.waterN;
    for (let i = 0; i < n; i++) {
      const x = xz[i*2], z = xz[i*2+1];
      wa[i*3+1] = Math.sin(x*0.72+t*1.65)*0.045 + Math.cos(z*0.52+t*1.28)*0.028;
    }
    refs.water.geometry.attributes.position.needsUpdate = true;
    // computeVertexNormals intentionally removed — ~1-2ms saved per frame
  }
